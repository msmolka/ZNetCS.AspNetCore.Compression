// --------------------------------------------------------------------------------------------------------------------
// <copyright file="CompressionExecutor.cs" company="Marcin Smółka">
...
public class CompressionExecutor
{
    ...
    /// The logger.
    /// </param>
    public CompressionExecutor(ILogger<CompressionExecutor> logger) => this.logger = logger;
    ...
    public async Task ExecuteAsync(HttpContext context, Stream bufferedStream, ICollection<ICompressor> compressors, CancellationToken cancellationToken)
    {
        ...
        if (compressor != null)
        {
            this.logger.Compressing(compressor.ContentCoding);
            ...
            await using (var compressionStream = new MemoryStream())
            {
                ...
                string[] contentEncodings = context.Response.Headers.GetCommaSeparatedValues(HeaderNames.ContentEncoding) ?? Array.Empty<string>();
                ...
                string[] vary = context.Response.Headers.GetCommaSeparatedValues(HeaderNames.Vary) ?? Array.Empty<string>();
            ...
            this.logger.CompressingFinished();
        }
    }
    ...
    {
        string[] acceptEncodings = context.Request.Headers.GetCommaSeparatedValues(HeaderNames.AcceptEncoding) ?? Array.Empty<string>();
